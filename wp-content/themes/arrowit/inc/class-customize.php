 ...
 */
if ( ! class_exists( 'Arrowit_Customize' ) ) {
	class Arrowit_Customize {
		...
		public function change_inline_stylesheet() {
			return 'arrowit-style';
		}
		/**
		 * Add customize preview css
		 */
		public function customize_preview_css() {
			wp_enqueue_style( 'kirki-custom-css', ARROWIT_THEME_URI . '/assets/admin/css/customizer.min.css' );
		}
		/**
		 * Load Customizer.
		 */
		public function load_customizer() {
			Arrowit::require_file( ARROWIT_THEME_DIR . '/customizer/customizer.php' );
		}
		/**
		 * Remove unused native sections and controls
		 *
			...
			if ( ! is_customize_preview() && $count === 2 ) {
				...
				if ( ! empty( $presets ) ) {
					foreach ( $presets as $preset ) {
						$page_preset_value = Arrowit_Helper::get_post_meta( $preset, '-1' );
