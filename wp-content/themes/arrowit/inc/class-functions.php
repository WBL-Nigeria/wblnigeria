...
if ( ! class_exists( 'Arrowit_Functions' ) ) {
	class Arrowit_Functions {
		...
		public function scroll_top() {
			?>
			<?php if ( Arrowit::setting( 'scroll_top_enable' ) && !is_page_template('coming-soon.php') && !is_404() ) : ?>
		...
		/* Preloader */
        public static function arrowit_pre_loader() {
		    $show_preloader = Arrowit::setting( 'preloader' );
		    $preloader = '';
            $arrowit_preloader = get_post_meta(get_the_ID(), 'preload', true);
            if( ($arrowit_preloader !='') && ($arrowit_preloader !='default')){
                $preloader = $arrowit_preloader;
            }else{
                $preloader = Arrowit::setting( 'preloader-image' );
            }
            if ($show_preloader === '1'): ?>
            <div class="preloader">
